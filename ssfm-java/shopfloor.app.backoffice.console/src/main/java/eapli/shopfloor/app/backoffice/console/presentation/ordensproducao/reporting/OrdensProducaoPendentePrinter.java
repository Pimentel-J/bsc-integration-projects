...
/**
 * Printer - Consultar Ordens de Produção Pendentes
 *
 *João Pimentel 
 */
public class OrdensProducaoPendentePrinter implements Visitor<OrdensProducao> {
    ...
    public void visit(final OrdensProducao visitee) {
        ...
        System.out.printf("%-15s%-15s%-20s%-15s%-15s%-15s%-35s", visitee.codigo, dataEmissao, dataPrevistaExec,
                visitee.produto.descricaoBreveProduto(), visitee.produto.identity(),
                String.format("%.0f", visitee.quantidade), visitee.estado);
    }
}

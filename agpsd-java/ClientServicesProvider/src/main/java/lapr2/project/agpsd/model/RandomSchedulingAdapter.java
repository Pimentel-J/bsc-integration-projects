...
import lapr2.project.agpsd.model.ServiceRequest.RequestStatus;

/**
 *
 *
 */
public class RandomSchedulingAdapter implements PairingAlgorithm {
    
    private RandomScheduling algorithm;

    public RandomSchedulingAdapter() {
        this.algorithm = new RandomScheduling();
    }
    ...
    public List<Pair<String, ProviderPairing>> getProviderPairings(ServiceRequestRegistry oRequestReg, ServiceProviderRegistry oProviderReg) {
        ...
        for (ServiceRequest request : oRequestReg.getServiceRequests()) {
            if (request.getReqStatus() == RequestStatus.UNPAIRED &&
